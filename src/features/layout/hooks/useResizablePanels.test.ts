...
describe("useResizablePanels", () => {
  ...
  it("reads stored sizes and clamps to bounds", () => {
    window.localStorage.setItem("agentsmanager.sidebarWidth", "999");
    window.localStorage.setItem("agentsmanager.rightPanelWidth", "100");
    window.localStorage.setItem("agentsmanager.planPanelHeight", "not-a-number");
  ...
  it("persists sidebar width changes and clamps max", () => {
    ...
    expect(hook.result.sidebarWidth).toBe(420);
    expect(window.localStorage.getItem("agentsmanager.sidebarWidth")).toBe(
      "420",
    );
